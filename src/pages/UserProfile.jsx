  ...
  return (
    <section className="profile">
      <div className="container profile__container">
        <Link to={`/myposts/${currentUser.id}`} className='btn bg-slate-50 hover:bg-black hover:text-white'>My Posts</Link>
        <div className="profile__details w-full">
          <div className="avatar__wrapper">
            ...
            {/* form */}
            <form className="avatar__form h-4">
              <input type="file" name='avatar' id='avatar' onChange={e => setAvatar(e.target.files[0])} accept='png, jpg, jpeg' className='hidden' />
              <label htmlFor="avatar" onClick={() => setIsAvatarTouched(true)}><FaEdit /></label>
            </form>
            {isAvatarTouched && <button className='profile__avatar-btn' onClick={changeAvatarHandler}><FaCheck /></button>}
          </div>

          <h1 className='mt-6'>{currentUser.name}</h1>
          ...
          <form action="" className="form profile__form" onSubmit={updateUserDetails}>
            ...
            <button type='submit' className='btn blue__btn'>Update Details</button>
